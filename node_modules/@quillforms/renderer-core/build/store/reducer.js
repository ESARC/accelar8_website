    ...
    case _constants.GO_NEXT:
      {
        ...
        const isThereNextField = walkPath.filter(block => block.name !== 'welcome-screen' && block.name !== 'thankyou-screen' && block.id === nextBlockId).length > 0;
        const isReallyLastField = walkPath?.length > 0 && walkPath[walkPath.length - 1].id === currentBlockId && !isThereNextField;
...
var _default = RendererCoreReducer;
